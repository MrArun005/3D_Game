...
test('every chunk: indexed, finite, unit normals, real surf kinds, wound to its normals, metre UVs', () => {
  let wrong = 0, total = 0, zeroUv = 0, verts = 0, displays = 0, sashes = 0;
  for (const c of chunks) {
    ...
    for (const v of S) { const k = Math.floor(v), f = v - k; if (k < SURF.WALL || k > SURF.SASH || f < 0.04 || f > 0.86) assert.fail(`${c.k}: surf ${v}`); if (k === SURF.DISPLAY) displays++; if (k === SURF.SASH) sashes++; }
  ...
  assert.ok(sashes > 0, 'no upper window carries SURF.SASH (the navy panes are back)');
});
