...
const COUNT = 96;
const SPAWN_MIN = 25, SPAWN_MAX = 190, DESPAWN = 260;
  ...
  update(car, dt, onHit) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      if (p.down) {
        ...
        const nx = p.x + Math.cos(p.yaw) * p.speed * dt;
        const nz = p.z - Math.sin(p.yaw) * p.speed * dt;
      ...
      if (!p.down) p.phase += p.speed * dt * 2.6;
